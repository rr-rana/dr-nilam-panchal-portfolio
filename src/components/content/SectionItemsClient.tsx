...
const SectionItemsClient = ({
  ...
  const fallbackHeaderLeft = (
    <div>
      <h1 className="public-hero-title text-3xl font-bold text-[#17323D]">{title}</h1>
      <p className="mt-2 text-sm text-[#4f6271]">
    ...
    return (
      <div className="public-page">
        <div className="mx-auto flex min-h-screen max-w-6xl items-center justify-center px-4">
          <div className="public-card rounded-3xl px-6 py-4">
            <LoadingSpinner />
          </div>
        </div>
      </div>
            ...
            {showHeader && (
              <section className="public-card rounded-[1.8rem] p-6">
                <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                  ...
                  {showSubmenuLinks && (
                    <div className="rounded-[1.3rem] border border-[#e6edf1] bg-[#edf3f5] p-4">
                      <h2 className="text-[11px] font-semibold uppercase tracking-[0.25em] text-[#b86d3a]">
                        Submenus
                      </h2>
                        ...
                        {isLoadingSubmenus ? (
                          <>
                            {Array.from({ length: 4 }).map((_, index) => (
                              <div
                                key={index}
                                className="skeleton-shimmer h-10 rounded-2xl border border-white/80"
                              />
                            ))}
                          </>
                        ) : submenus.length === 0 ? (
                          <div className="rounded-2xl border border-dashed border-[#cad6dd] bg-white/70 px-4 py-6 text-center text-xs text-[#4c5f66]">
                            ...
                            <Link
                              ...
                              className={`group flex items-center justify-between rounded-2xl border px-4 py-3 text-sm font-semibold transition-all ${
                                submenu.slug === activeSubmenu
                                  ...
                                  : "border-white/80 bg-white text-[#17323D] hover:-translate-y-0.5 hover:border-[#17323D]/25 hover:text-[#b86d3a] hover:shadow-md"
                              }`}
                            >
            ...
            {showBackButton && (
              <section>
                <Link
                  ...
                  className="inline-flex items-center rounded-full border border-[#d2dde4] bg-white px-4 py-2 text-xs font-semibold text-[#17323D] shadow-sm transition-colors hover:bg-[#eef3f6]"
                >
                  {backLabel}
                </Link>
              </section>
              ...
              {!activeSubmenu || !showItems ? null : isLoadingItems || isLoadingContent ? (
                <>
                  {Array.from({ length: 3 }).map((_, index) => (
                    <div
                      key={index}
                      className="public-card rounded-3xl p-4 sm:p-5"
                    >
                ...
                <div className="public-card rounded-3xl p-8">
                  <div className="mx-auto flex max-w-xl flex-col items-center gap-4 text-center">
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-[#d2dfe6] bg-[#edf3f5] text-[#17323D] shadow-inner">
                      <FolderOpen size={24} />
                    </div>
                    ...
                    <div className="w-full max-w-sm space-y-2">
                      <div className="mx-auto h-3 w-3/4 rounded-full bg-[#dbe6ec]" />
                      <div className="mx-auto h-3 w-full rounded-full bg-[#eaf1f4]" />
                      <div className="mx-auto h-3 w-2/3 rounded-full bg-[#dbe6ec]" />
                    </div>
                  </div>
                </div>
              ) : (
                paginationState.pagedItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
                        <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl border border-[#d9e4eb] bg-[#edf3f5] sm:h-24 sm:w-24">
                          {thumbnail ? (
                            <Image
                              src={thumbnail.url}
                            ...
                            <div className="flex h-full w-full items-center justify-center text-[10px] font-semibold uppercase tracking-[0.2em] text-[#617586]">
                              No Image
                            </div>
                          )}
                        </div>
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-2 line-clamp-3 text-sm text-[#4f6271]">
                            {preview || "No description yet."}
                          </p>
                          <div className="mt-3 flex flex-wrap items-center gap-3">
                            <Link
                              href={`${basePath}/${activeSubmenu}/${item.id}`}
                              className="inline-flex items-center gap-2 rounded-full bg-[#17323D] px-4 py-2 text-xs font-semibold text-white shadow-md shadow-[#17323D]/30"
                            >
                              <Eye size={14} />
                              View Details
                            </Link>
                            {item.pdfUrl && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center gap-2 rounded-full border border-[#d1dee6] bg-[#f4f8fa] px-4 py-2 text-xs font-semibold text-[#17323D]"
                              >
                                <FileText size={14} />
                                Download PDF
                              </a>
                        ...
                        {showMeta && (
                          <div className="text-xs text-[#4f6271] sm:min-w-[160px] sm:text-right">
                            {author && (
                              <>
              ...
              <section className="flex items-center justify-center gap-2">
                <Link
                  ...
                  className={`flex h-8 w-8 items-center justify-center rounded-full border text-xs font-semibold transition-colors ${
                    paginationState.currentPage === 1
                      ? "pointer-events-none border-[#d7e1e8] bg-white/70 text-[#9aa3a8]"
                      : "border-[#d1dee6] bg-white text-[#17323D] hover:bg-[#edf3f6]"
                  }`}
